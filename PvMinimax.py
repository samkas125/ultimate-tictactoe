...
from additional import minimax
pygame.init()
...
pygame.display.set_caption('Ultimate Tic-Tac-Toe (PvMinimax)')
...
def main():
    ...
    while run:
        ...
        if curr_player == -1:
            copy = state.copy()
            minimax_state = game.change_perspective(copy, -1)

            if ALPHA_BETA_PRUNING:
                _, action = minimax.minimax_alphabeta(minimax_state, depth, True, -float('inf'), float('inf'))
            else:
                _, action = minimax.minimax(minimax_state, depth, True)
